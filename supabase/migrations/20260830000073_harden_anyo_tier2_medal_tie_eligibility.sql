-- ============================================================================
-- Migration: 20260830000073_harden_anyo_tier2_medal_tie_eligibility.sql
-- Description: Harden record_anyo_score with server-authoritative medal-tie eligibility for Tier 2.
-- Phase: P-ANYO-TIER2-SERVER-MEDAL-TIE-GUARD-06B
-- Finding: F-ANYO-T2-MEDAL-TIE-SERVER-CHECK
-- ============================================================================
...
BEGIN
  ...
  -- 5. Tier 2 Defensive Invariants: Tier 1 prerequisite & Medal-tie eligibility
  IF p_tier = 'TIER_2' THEN
    -- Gate A: Tier 1 score must exist before recording Tier 2
    IF NOT EXISTS (
      SELECT 1
    ...
    -- Gate B: Server-authoritative medal-tie eligibility check
    -- Performance must currently belong to an unresolved medal-contending tie (Rank <= 3, cluster_size > 1)
    IF NOT EXISTS (
      WITH session_tier1_scores AS (
        SELECT 
          p.id AS perf_id,
          s.calculated_score AS final_score,
          DENSE_RANK() OVER (ORDER BY s.calculated_score DESC) AS score_rank,
          COUNT(*) OVER (PARTITION BY s.calculated_score) AS cluster_size
        FROM public.anyo_performances p
        JOIN public.anyo_scores s ON s.performance_id = p.id AND s.tier = 'TIER_1'
        WHERE p.session_id = v_perf.session_id
          AND p.status IN ('COMPLETED', 'PERFORMING')
      )
      SELECT 1 
      FROM session_tier1_scores
      WHERE perf_id = p_performance_id
        AND cluster_size > 1
        AND score_rank <= 3
    ) THEN
      RAISE EXCEPTION 'INVALID_ARGUMENT: Tier 2 re-performance is permitted only for legitimate medal-contending ties'
        USING ERRCODE = '22000';
    END IF;
  END IF;
