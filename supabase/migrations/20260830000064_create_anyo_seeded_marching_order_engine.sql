-- Migration: 20260830000064_create_anyo_seeded_marching_order_engine.sql
-- Description: ANYO Achievement-Based Seeded Marching Order Engine with 24-Month Recency, Category Isolation & Snapshot Cutoff
-- Target Domain: UAAPHIL Tournament System (Forms / ANYO Engine)
-- Sequence: 000064 (Strictly Additive, Preserves Full Contact Sparring Engine & Existing Migrations)

-- ============================================================================
-- 1. SCHEMA EXTENSIONS (ADDITIVE & NON-DESTRUCTIVE)
-- ============================================================================

-- A. anyo_performances additions
ALTER TABLE public.anyo_performances
  ADD COLUMN IF NOT EXISTS seed_tier INT NOT NULL DEFAULT 5,
  ADD COLUMN IF NOT EXISTS historical_classification TEXT NOT NULL DEFAULT 'UNSEEDED',
  ADD COLUMN IF NOT EXISTS seed_details JSONB NOT NULL DEFAULT '{}'::jsonb,
  ADD COLUMN IF NOT EXISTS draw_group TEXT,
  ADD COLUMN IF NOT EXISTS seeding_cutoff_at TIMESTAMPTZ;

-- B. anyo_category_sessions additions
ALTER TABLE public.anyo_category_sessions
  ...
  ADD COLUMN IF NOT EXISTS draw_generated_at TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS draw_confirmed_at TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS draw_confirmed_by UUID REFERENCES public.profiles(id),
  ADD COLUMN IF NOT EXISTS draw_metadata JSONB NOT NULL DEFAULT '{}'::jsonb;

-- Indexes for performance query and seeding lookups
CREATE INDEX IF NOT EXISTS idx_anyo_performances_session_order 
  ON public.anyo_performances(session_id, order_number);

CREATE INDEX IF NOT EXISTS idx_anyo_performances_reg_status 
  ON public.anyo_performances(registration_id, status);

CREATE INDEX IF NOT EXISTS idx_anyo_category_sessions_draw_status 
  ON public.anyo_category_sessions(tournament_id, draw_status);

-- ============================================================================
-- 2. HISTORICAL SEEDING LOOKUP HELPER
-- ============================================================================
...
BEGIN
  ...
  -- 4. Timestamp within [v_window_start, p_cutoff]
  FOR v_rec IN (
    SELECT 
...
-- ============================================================================
-- 3. RPC: generate_anyo_marching_order
-- ============================================================================
...
BEGIN
  ...
  -- 5. Performer Status Validation (No regeneration once any performance is active)
  ...
  -- 6. Check locked confirmation state
  IF NOT p_is_regeneration AND v_session.draw_status = 'CONFIRMED' THEN
    RAISE EXCEPTION 'INVALID_STATE: Official marching order is already CONFIRMED. Use regeneration flag if intentional re-draw is needed.'
      USING ERRCODE = '22000';
  END IF;
  ...
  -- 8. Temporary Table for Staging and Controlled Randomization
  ...
  -- 9. Evaluate each registered competitor against historical records
  FOR v_perf IN (
    SELECT 
      ap.id AS perf_id,
      r.user_id,
    ...
    -- Insert into staging pool with server-side random weight
  ...
  -- Within each tier: Server-side randomized (random_weight ASC)
  FOR v_perf IN (
    SELECT 
      t.perf_id,
      t.seed_tier,
...
-- ============================================================================
-- 4. RPC: confirm_anyo_marching_order
-- ============================================================================

CREATE OR REPLACE FUNCTION public.confirm_anyo_marching_order(
  p_session_id UUID
)
RETURNS JSONB
LANGUAGE plpgsql
...
BEGIN
  ...
  IF NOT public.is_authorized_tournament_official(v_requester_id, v_session.tournament_id, v_session.event_id) THEN
    RAISE EXCEPTION 'FORBIDDEN: Requester is not authorized for this session'
      USING ERRCODE = '40300';
  END IF;

  IF v_session.status <> 'SCHEDULED' THEN
    RAISE EXCEPTION 'INVALID_STATE: Session is in % status. Confirmation only valid in SCHEDULED status.', v_session.status
      USING ERRCODE = '22000';
  END IF;

  IF v_session.draw_status <> 'GENERATED' THEN
    RAISE EXCEPTION 'INVALID_STATE: Marching order must be GENERATED before it can be confirmed. Current: %', v_session.draw_status
      USING ERRCODE = '22000';
  END IF;

  SELECT COUNT(*) INTO v_active_count
  FROM public.anyo_performances
  WHERE session_id = p_session_id AND status <> 'WAITING'::public.anyo_performance_status;

  IF v_active_count > 0 THEN
    RAISE EXCEPTION 'INVALID_STATE: Cannot modify confirmation state because competition has started with active performers.'
      USING ERRCODE = '22000';
  END IF;
  ...
  SET 
    ...
    draw_confirmed_at = timezone('utc'::text, now()),
    draw_confirmed_by = v_requester_id,
    updated_at = timezone('utc'::text, now())
  WHERE id = p_session_id;
  ...
  INSERT INTO public.system_audit_logs (
    ...
    v_requester_id,
    'OFFICIAL',
    'CONFIRM_ANYO_MARCHING_ORDER',
    'ANYO_SESSION',
    p_session_id,
    v_session.tournament_id,
    jsonb_build_object(
      'session_id', p_session_id,
      'confirmed_by', v_requester_id,
...
-- ============================================================================
-- 5. RPC: reorder_anyo_performances (HARDENED WITH REASON & AUDIT DIFF)
-- ============================================================================

CREATE OR REPLACE FUNCTION public.reorder_anyo_performances(
  p_session_id UUID,
  p_performance_ids UUID[],
  p_reason TEXT DEFAULT 'MANUAL_OFFICIAL_OVERRIDE'
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Lifecycle & Active Performer Guard
  SELECT COUNT(*) INTO v_active_count
  FROM public.anyo_performances
  WHERE session_id = p_session_id AND status <> 'WAITING'::public.anyo_performance_status;
  ...
  -- Capture Before-Order Snapshot for Audit Trail
  SELECT jsonb_agg(
    jsonb_build_object(
      'performance_id', id,
  ...
  INSERT INTO public.system_audit_logs (
    ...
    v_session.tournament_id,
    jsonb_build_object(
      'reason', COALESCE(p_reason, 'MANUAL_OFFICIAL_OVERRIDE'),
...
-- ============================================================================
-- 6. RPC: get_anyo_marching_order_preview
-- ============================================================================
...
BEGIN
  SELECT * INTO v_session FROM public.anyo_category_sessions WHERE id = p_session_id;
  IF v_session.id IS NULL THEN
    RAISE EXCEPTION 'NOT_FOUND: Anyo session % not found', p_session_id USING ERRCODE = 'P0002';
  END IF;
...
GRANT EXECUTE ON FUNCTION public.get_anyo_marching_order_preview(UUID) TO authenticated, anon, service_role;
